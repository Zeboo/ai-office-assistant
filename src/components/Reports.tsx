...
function Reports({
  ...
  const mapBackendReport = (report: BackendReport): Report => ({
    id: report.id,
    title: report.title,
    type: report.type,
    date: report.date,
  ...
  const createReport = async () => {
    ...
    try {
      ...
      const createdReport = mapBackendReport(data.report);
      ...
      setTimeout(async () => {
        try {
          const completeResponse = await fetch(
            `http://localhost:3000/reports/${createdReport.id}/complete`,
            {
              method: "PATCH",
            },
          );
          ...
          const completedReport = mapBackendReport(
            completeData.report,
          );
  ...
  const refreshProcessing = async () => {
    ...
    try {
      await Promise.all(
        processing.map(async (report) => {
          const response = await fetch(
            `http://localhost:3000/reports/${report.id}/complete`,
            {
              method: "PATCH",
            },
          );
          ...
          if (!response.ok) {
            throw new Error(
              `Failed to complete report ${report.id}.`,
            );
          }
          ...
          const completedReport = mapBackendReport(data.report);

          setReports((current) =>
            current.map((currentReport) =>
              currentReport.id === completedReport.id
                ? completedReport
                : currentReport,
            ),
          );
      ...
      console.error("Refresh processing error:", error);
    }
  };
  ...
  return (
    ...
    >
      ...
      <div className="mb-6 grid grid-cols-3 gap-5">
        ...
        >
          <h2 className="text-sm font-semibold">Workspace Score</h2>
